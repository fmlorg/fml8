...
#
# $FML: htmlify.pm,v 1.4 2002/04/07 05:01:33 fukachan Exp $
#
...
sub process
{
    ...
    # override
    $debug   = 1 if defined $options->{ debug } || defined $options->{ d };
    $src_dir = $options->{ srcdir } if defined $options->{ srcdir };
    $dst_dir = $options->{ dstdir } if defined $options->{ dstdir };
